...
<body class="bg-base-100">
      ...
      <main class="flex-1 overflow-y-auto bg-gray-50 p-4 sm:p-6">
        ...
        <div class="mb-6">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h1 class="text-2xl font-bold text-gray-800">Visitor Logs & Analytics</h1>
                <p class="text-gray-500 text-sm">Comprehensive visitor tracking, analytics, and reporting</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="mb-6">
          <div class="flex flex-wrap gap-2">
            <button id="nav-logs" onclick="showTab('logs')"
              class="tab-btn flex items-center gap-2.5 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 {{ $activeTab === 'logs' ? 'bg-[#001F54] text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200' }}">
              <div
                class="w-7 h-7 rounded-lg {{ $activeTab === 'logs' ? 'bg-white/20' : 'bg-[#001F54]' }} flex items-center justify-center">
                <i data-lucide="list" class="w-3.5 h-3.5 text-[#F7B32B]"></i>
              </div>
              Detailed Logs
            </button>
            <button id="nav-reports" onclick="showTab('reports')"
              class="tab-btn flex items-center gap-2.5 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 {{ $activeTab === 'reports' ? 'bg-[#001F54] text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200' }}">
              <div
                class="w-7 h-7 rounded-lg {{ $activeTab === 'reports' ? 'bg-white/20' : 'bg-[#001F54]' }} flex items-center justify-center">
                <i data-lucide="bar-chart-3" class="w-3.5 h-3.5 text-[#F7B32B]"></i>
              </div>
              Reports & Analytics
            </button>
          </div>
        ...
        <div>
          <!-- Detailed Logs Tab -->
          <div id="logs-content" class="visitor-tab-panel">
            <!-- Logs Table -->
            <x-table-card :title="'Visitor Logs'">
                ...
                <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                  <div class="flex flex-1 gap-3 items-center w-full sm:w-auto">
                    ...
                    <div class="relative flex-1 sm:flex-initial sm:w-64">
                      ...
                      <input type="text" id="logs-search-input" placeholder="Search..."
                    ...
                    <div class="dropdown dropdown-bottom">
                      <label tabindex="0"
                        class="btn btn-square border-none shadow-sm hover:brightness-95 transition-all duration-200"
                        style="background-color: #F7A923; color: #001F54;" title="Filter by Time">
                        <i data-lucide="filter" class="w-5 h-5"></i>
                      </label>
                      <ul tabindex="0"
                        class="dropdown-content z-[1] menu p-2 shadow-lg bg-white rounded-xl w-52 mt-2 border border-blue-50">
                        <li class="menu-title px-4 py-2 text-xs font-bold text-gray-400 uppercase tracking-wider">Time
                          Range</li>
                        <li><a onclick="setDateRange('all')"
                            class="active:bg-[#001F54] active:text-white hover:bg-blue-50 rounded-lg mb-1"><i
                              data-lucide="infinity" class="w-4 h-4"></i> All Time</a></li>
                        <li><a onclick="setDateRange('today')" class="hover:bg-blue-50 rounded-lg mb-1"><i
                              data-lucide="calendar" class="w-4 h-4"></i> Today</a></li>
                        <li><a onclick="setDateRange('week')" class="hover:bg-blue-50 rounded-lg mb-1"><i
                              data-lucide="calendar-days" class="w-4 h-4"></i> Last 7 Days</a></li>
                        <li><a onclick="setDateRange('month')" class="hover:bg-blue-50 rounded-lg mb-1"><i
                              data-lucide="calendar-range" class="w-4 h-4"></i> Last 30 Days</a></li>
                        <li><a onclick="setDateRange('custom')" class="hover:bg-blue-50 rounded-lg"><i
                              data-lucide="calendar-plus" class="w-4 h-4"></i> Custom Range</a></li>
                      </ul>
                    </div>
                  </div>
                  ...
                  <div class="flex gap-4 items-center">
                    ...
                    <div class="flex flex-col items-center">
                      <button type="button" id="logs-export-btn"
                        ...
                        onclick="exportVisitorLogs()" title="Export Logs">
                ...
                <thead>
                  <tr class="bg-gray-50 border-b border-gray-100">
                    ...
                    <th class="text-center py-3 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider w-44"
                      data-column="checkin_checkout">Check-In / Out</th>
                    <th class="text-center py-3 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider w-28"
                      data-column="approval_status">Status</th>
                    <th class="text-center py-3 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider w-32"
                      data-column="id_number">Pass ID</th>
                    <th class="text-center py-3 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider w-28"
                      data-column="actions">Actions</th>
                  </tr>
                </thead>
                <tbody id="logs-table-body">
                  @forelse($visitors as $visitor)
                    <tr
                      class="hover:bg-blue-50/30 transition-colors duration-200 border-b border-gray-100 interactive-row"
                      data-id="{{ $visitor->id }}" data-name="{{ $visitor->name ?? 'Visitor' }}"
                      data-email="{{ $visitor->email ?? 'N/A' }}" data-phone="{{ $visitor->contact ?? 'N/A' }}"
                      data-host="{{ $visitor->host_employee ?? $visitor->host ?? 'N/A' }}"
                      data-source="{{ $visitor->source ?? 'N/A' }}" data-checkin="{{ $visitor->time_in ?? '' }}"
                      data-checkout="{{ $visitor->time_out ?? '' }}" data-purpose="{{ $visitor->purpose ?? 'N/A' }}"
                      ...
                      data-pass-id="{{ $visitor->pass_id ?? 'N/A' }}" data-company="{{ $visitor->company ?? '' }}"
                        ...
                        <div class="flex items-center gap-3">
                          <div class="relative flex-shrink-0">
                            @if(!empty($visitor->profile_photo_url))
                              <div class="w-10 h-10 rounded-lg overflow-hidden ring-2 ring-[#001F54]/20">
                                <img src="{{ $visitor->profile_photo_url }}" alt="{{ $visitor->name }}"
                                  class="object-cover w-full h-full">
                              </div>
                            @else
                              <div class="w-10 h-10 rounded-lg bg-[#001F54] flex items-center justify-center">
                                <span
                                  class="text-sm font-semibold text-[#F7B32B]">{{ strtoupper(substr($visitor->name ?? 'V', 0, 1)) }}</span>
                              </div>
                            @endif
                            @if(!empty($visitor->nationality_flag))
                              <img src="{{ $visitor->nationality_flag }}" alt="Flag"
                                class="w-4 h-4 absolute -right-1 -bottom-1 rounded-full border border-white">
                            @endif
                          </div>
                          <div class="min-w-0">
                            <p class="font-medium text-gray-800 text-sm">{{ $visitor->name ?? 'Visitor' }}</p>
                            <p class="text-xs text-gray-400 truncate max-w-[150px]">{{ $visitor->email ?? 'No email' }}
                            </p>
                          </div>
                        </div>
                      </td>
                      ...
                      <!-- Contact -->
                      <td class="py-4 px-4 text-center text-sm text-gray-600" data-column="contact_number">
                        {{ $visitor->contact ?? 'N/A' }}
                      </td>

                      <!-- Purpose -->
                      <td class="py-4 px-4 text-center" data-column="purpose">
                        <span
                      ...
                      <!-- Check-In / Check-Out -->
                      <td class="py-4 px-4 text-center" data-column="checkin_checkout">
                        @php
                      ...
                      @php
                        ...
                        $statusConfig = match ($approvalStatus) {
                          'COMPLETED' => ['bg' => 'bg-emerald-50', 'text' => 'text-emerald-700', 'border' => 'border-emerald-200', 'icon' => 'check-circle'],
                          'APPROVED' => ['bg' => 'bg-emerald-50', 'text' => 'text-emerald-700', 'border' => 'border-emerald-200', 'icon' => 'check-circle'],
                          'REJECTED' => ['bg' => 'bg-red-50', 'text' => 'text-red-700', 'border' => 'border-red-200', 'icon' => 'x-circle'],
                          default => ['bg' => 'bg-amber-50', 'text' => 'text-amber-700', 'border' => 'border-amber-200', 'icon' => 'clock']
                        };
                      @endphp
                      <td class="py-4 px-4 text-center" data-column="approval_status">
                        <span
                          class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium {{ $statusConfig['bg'] }} {{ $statusConfig['text'] }} border {{ $statusConfig['border'] }}">
                      ...
                      <!-- Pass ID -->
                      <td class="py-4 px-4 text-center" data-column="id_number">
                        <span
                          class="text-xs font-mono text-blue-600 bg-blue-50 px-2 py-1 rounded">{{ $visitor->pass_id ?? 'N/A' }}</span>
                      </td>

                      <!-- Actions -->
                      <td class="py-4 px-4" data-column="actions">
                        <div class="flex items-center justify-center gap-1">
                          <button
                            class="w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-200 hover:scale-110 bg-red-50 text-red-500 hover:bg-red-100"
                            title="Report violation"
                            onclick="event.stopPropagation(); reportViolation({{ $visitor->id }})">
                            <i data-lucide="alert-triangle" class="w-4 h-4"></i>
                          </button>
                          <button
                            class="w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-200 hover:scale-110 bg-emerald-50 text-emerald-600 hover:bg-emerald-100"
                            title="Print pass" onclick="event.stopPropagation(); printVisitorPass({{ $visitor->id }})">
                          ...
                          <button
                            ...
                            title="Check-out"
                            onclick="event.stopPropagation(); openCheckoutRatingModal({{ $visitor->id }})">
                    ...
                    <tr>
                      <td colspan="7" class="py-16 text-center">
                        <div class="flex flex-col items-center">
                          ...
                          <h3 class="text-lg font-semibold text-gray-700 mb-1">No Visitor Logs Found</h3>
                          <p class="text-sm text-gray-500">No visitor logs available for the selected criteria.</p>
                        </div>
                      </td>
                    </tr>
                  @endforelse
              ...
              <div class="table-footer">
                ...
                <div class="flex items-center gap-3 text-gray-500">
                  <button id="logs-prev-btn" onclick="goToPage({{ $visitors->currentPage() - 1 }})"
                    class="p-1 disabled:opacity-30 disabled:cursor-not-allowed hover:text-gray-700 transition-colors" {{ $visitors->onFirstPage() ? 'disabled' : '' }} aria-label="Previous page">
                    <i data-lucide="chevron-left" class="w-4 h-4"></i>
                  </button>
                  <button id="logs-next-btn" onclick="goToPage({{ $visitors->currentPage() + 1 }})"
                    class="p-1 disabled:opacity-30 disabled:cursor-not-allowed hover:text-gray-700 transition-colors" {{ !$visitors->hasMorePages() ? 'disabled' : '' }} aria-label="Next page">
          ...
          <div id="reports-content" class="visitor-tab-panel hidden">
            <!-- Header with Export Button -->
            <div class="flex justify-between items-center mb-6">
              <div>
                <h2 class="text-2xl font-bold text-gray-800" style="color: var(--color-charcoal-ink);">Reports &
                  Analytics</h2>
                <p class="text-gray-600" style="color: var(--color-charcoal-ink); opacity: 0.8;">Comprehensive visitor
                  management insights and statistics</p>
              </div>
              <div class="flex items-center gap-3">
                <div class="flex items-center gap-2">
                  <i data-lucide="calendar" class="w-4 h-4 text-gray-500"></i>
                  <select class="select select-bordered select-sm" id="analytics-time-range"
                ...
                <button onclick="refreshAnalytics()" class="btn btn-sm"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              ...
              <div class="reports-card bg-white rounded-xl p-6 shadow-lg border border-gray-100">
                <div class="flex items-center justify-between mb-3">
                  ...
                  <span class="text-xs font-medium text-green-600 bg-green-100 px-2 py-1 rounded-full badge-pulse">+12%
                    from last week</span>
                </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3" id="columnCheckboxContainer">
          @foreach($columnOptions as $option)
            <label class="flex items-center gap-2 text-sm font-medium text-gray-700 cursor-pointer select-none">
              <input type="checkbox" class="checkbox checkbox-warning column-visibility-checkbox"
                data-column-id="{{ $option['id'] }}" checked>
      ...
      <div class="bg-white rounded-2xl border-2 border-gray-200 shadow-lg overflow-hidden">
        ...
        <div class="bg-gradient-to-br from-indigo-600 via-purple-600 to-blue-600 px-6 py-8 text-center">
          ...
          <div class="flex justify-center mb-3">
            <div
              class="w-24 h-24 rounded-full overflow-hidden border-4 border-white/30 bg-white/20 backdrop-blur-sm flex items-center justify-center">
              <img id="printPassPhoto" src="" alt="Visitor photo" class="w-full h-full object-cover"
          ...
          <!-- Status Badge -->
          <span id="printPassStatus"
            class="inline-block px-4 py-1 rounded-full text-xs font-semibold bg-red-500 text-white">EXPIRED</span>
        </div>
        ...
        <div class="px-6 py-6 space-y-4">
          ...
          <div class="flex flex-col items-center pt-4 border-t border-gray-200">
            ...
            <div
              class="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-center py-3 rounded-lg font-semibold text-sm">
          ...
          <div class="pt-4 border-t border-gray-200">
            <button type="button" id="toggleMoreDetails" onclick="toggleMoreDetails()"
  ...
  <div id="checkoutRatingModal" class="modal" onclick="closeCheckoutRatingModal()">
    <div class="modal-box w-full max-w-xl" onclick="event.stopPropagation()">
      ...
      <div class="mb-4">
        ...
        <textarea id="ratingComment" class="textarea textarea-bordered w-full" rows="3"
  ...
  <div id="reportViolationModal" class="modal" onclick="closeReportViolationModal()">
      ...
      <form id="violationReportForm" onsubmit="submitViolationReport(event)">
        ...
        <div class="bg-gray-50 rounded-lg p-4 mb-6">
          <div class="flex items-center gap-4">
            <div class="w-16 h-16 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center">
              <img id="violationVisitorPhoto" src="" alt="Visitor photo" class="w-full h-full object-cover"
                onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
              <div class="w-full h-full bg-blue-900 flex items-center justify-center text-white font-semibold hidden"
                id="violationVisitorInitial">
                V
              </div>
            </div>
            <div>
        ...
        <div class="mb-4">
          ...
          <input type="text" id="incidentLocation" name="incident_location" class="input input-bordered w-full"
            placeholder="e.g., Main Lobby, Building A, Floor 3">
        </div>
        ...
        <div class="mb-4">
          ...
          <textarea id="violationDescription" name="description" class="textarea textarea-bordered w-full" rows="5"
            placeholder="Provide a detailed description of the violation..." required></textarea>
        </div>
    ...
    <dialog id="bulkActionsModal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg mb-4 text-left">Bulk Visitor Actions</h3>
    ...
    <dialog id="createBulkSessionModal" class="modal">
      <div class="modal-box w-11/12 max-w-lg text-left">
    ...
    <dialog id="approveBulkSessionModal" class="modal">
      <div class="modal-box w-11/12 max-w-4xl max-h-[90vh] flex flex-col text-left">
    ...
    <script>
      ...
      function showTab(tabName) {
        ...
        document.querySelectorAll('.visitor-tab-panel').forEach(content => {
        ...
        [nav1, nav2].forEach(btn => setTabInactive(btn));
      ...
      function loadTabData(tabName) {
        switch (tabName) {
          case 'logs':
            // Don't fetch data if no filters are applied - use server-rendered data
            ...
            // Only fetch via AJAX if filters are applied
            // Otherwise, server-rendered data is already visible
            if (hasFilters) {
              loadLogsData();
            }
            // If no filters, do nothing - server-rendered data stays visible
            break;
          case 'reports':
            loadReportsData();
            break;
      ...
      function loadAnalyticsData() {
        // Check if reports tab content exists and is visible
        const reportsContent = document.getElementById('reports-content');
        if (!reportsContent || reportsContent.classList.contains('hidden')) {
          return; // Don't load if reports tab is not visible
        }
      ...
      function createDailyTrendsChart(data = null) {
        ...
        if (!canvas) {
          // Canvas not found - this is expected if charts are not shown
          return;
        }
      ...
      function applyCustomRange() {
        // Check if reports tab content exists
        const reportsContent = document.getElementById('reports-content');
        if (!reportsContent) {
          return;
        }
      ...
      function loadLogsData() {
        ...
        // Initialize pagination from server-side data (only if we haven't replaced server data yet)
        if (!hasReplacedServerData) {
          @if(isset($visitors))
        ...
        // Only fetch if we actually have filters to apply
        // This function should only be called when filters are applied
      ...
      function showLogsLoading() {
        ...
        // ONLY show loading if we've already replaced server data
        // NEVER clear server-rendered data on initial load
        if (hasReplacedServerData) {
          const existingRows = tbody.querySelectorAll('tr.interactive-row').length;
          // Only show loading spinner if we've already replaced the data
          if (existingRows > 0) {
            // Add loading overlay or spinner without clearing data
            // For now, just skip showing loading if data exists
            return;
          }
          tbody.innerHTML = '<tr><td colspan="8" class="text-center py-12"><div class="flex flex-col items-center"><div class="loading loading-spinner loading-lg mb-2"></div><p class="text-sm text-gray-500">Loading visitor logs...</p></div></td></tr>';
        }
        // If server data hasn't been replaced, don't show loading (preserve server-rendered data)
      }

      function hideLogsLoading() {
        // Loading is hidden when renderLogsTable is called
      }

      function renderLogsTable(rows = []) {
        const tbody = document.getElementById('logs-table-body');
        if (!tbody) {
          console.warn('[Visitor Logs Debug] renderLogsTable: tbody element not found');
          return;
        }

        // Debug: Log what we're about to render
        console.log('[Visitor Logs Debug] renderLogsTable called with', rows.length, 'rows');

        if (!Array.isArray(rows) || rows.length === 0) {
          // Only render empty state if we've actually replaced server data
          if (hasReplacedServerData) {
            tbody.innerHTML = '<tr><td colspan="8" class="text-center py-12"><div class="flex flex-col items-center"><div class="w-16 h-16 bg-blue-50 rounded-full flex items-center justify-center mb-4"><i data-lucide="clipboard-list" class="w-8 h-8 text-blue-300"></i></div><h3 class="text-lg font-semibold text-gray-700 mb-1">No Visitor Logs Found</h3><p class="text-sm text-gray-500">No visitor logs available for the selected criteria.</p></div></td></tr>';
            if (window.lucide && window.lucide.createIcons) { window.lucide.createIcons(); }
          } else {
            // Don't replace server-rendered data with empty state
            console.warn('[Visitor Logs Debug] renderLogsTable: Empty rows but server data not replaced, keeping server data');
          }
          return;
        }

        // CRITICAL: Only replace server-rendered data if we've explicitly fetched new data via AJAX
        // Mark that we're about to replace server data (only when we have actual data to render)
        hasReplacedServerData = true;
      ...
      let perPage = 10;
      let totalItems = 0;
      let hasReplacedServerData = false; // Track if we've replaced server-rendered data
      let isLoadingLogs = false; // Track loading state
      ...
      function fetchLogs(startDate, endDate, page = 1, itemsPerPage = null) {
        ...
        fetch(url.toString(), {
          ...
          .then(data => {
            isLoadingLogs = false;
            hideLogsLoading();
            hasReplacedServerData = true; // Mark that we've replaced server data
          ...
          .catch(err => {
            ...
            // Only clear table if we've already replaced server-rendered data
            // This preserves the initial server-rendered data on first load errors
            if (hasReplacedServerData) {
              renderLogsTable([]);
              console.error('Failed to fetch visitor logs:', err);
              if (typeof window.showNotification === 'function') {
                window.showNotification('Failed to load visitor logs. Please refresh the page.', 'error');
              }
            } else {
              // Keep server-rendered data, just log the error (don't show notification on initial load)
              console.warn('AJAX fetch failed, keeping server-rendered data:', err);
            }
            updatePaginationControls();
          });
      }
      ...
      // CRITICAL: Verify server-rendered data exists in DOM
      const tbody = document.getElementById('logs-table-body');
        const serverRenderedRows = tbody ? tbody.querySelectorAll('tr.interactive-row').length : 0;

        // Only load data via AJAX if filters are applied or reports tab is active
        // Otherwise, use the server-rendered data that's already in the table
        const startDate = document.getElementById('logs-start-date')?.value || '';
        const endDate = document.getElementById('logs-end-date')?.value || '';
        const hasFilters = startDate || endDate;

        // Debug: Log initial state
        console.log('[Visitor Logs Debug] Server-rendered rows:', serverRenderedRows);
        console.log('[Visitor Logs Debug] Has filters:', hasFilters);
        console.log('[Visitor Logs Debug] Current tab:', currentTab);

        // DO NOT call loadLogsData() on page load unless filters are explicitly applied
        // Server-rendered data must remain visible by default
        setTimeout(() => {
          // Only fetch logs via AJAX if filters are applied
          // The server-rendered data is already displayed, so we don't need to replace it
          if (hasFilters && currentTab === 'logs') {
            console.log('[Visitor Logs Debug] Filters detected, loading via AJAX...');
            loadLogsData();
          } else {
            console.log('[Visitor Logs Debug] No filters, preserving server-rendered data');
          }

          // Load analytics data only if reports tab is active
      ...
      async function processManualBulkToken() {
        ...
        try {
          ...
          console.error(err);
          showNotification('Error processing token', 'error');
        }
      }
      ...
      function showBulkApprovalModal(session, count) {
        ...
        if (visitors.length > 0) {
          visitors.forEach((v, index) => {
            ...
            row.innerHTML = `
              <td class="py-2.5 px-4 text-xs text-gray-400 font-mono text-left">${index + 1}</td>
              <td class="py-2.5 px-4 font-medium text-gray-800 text-sm text-left">${v.name}</td>
              <td class="py-2.5 px-4 text-xs text-gray-500 text-left">${v.email || '—'}</td>
              <td class="py-2.5 px-4 text-xs text-gray-500 text-left">${v.phone || '—'}</td>
              <td class="py-2.5 px-4 text-xs text-gray-500 text-left">${v.company || '—'}</td>
              <td class="py-2.5 px-4 text-xs text-gray-500 text-left">${v.host || '—'}</td>
          `;
            tbody.appendChild(row);
          });
        } else {
          const row = document.createElement('tr');
          row.innerHTML = `<td colspan="6" class="py-3 px-4 text-center text-gray-400 text-sm italic">Guest 1 to ${count} - ${session.group_name} (Auto-generated entry)</td>`;
          tbody.appendChild(row);
        }
      ...
      async function confirmBulkApproval() {
        ...
        try {
          ...
          const data = await res.json();
          if (data.success) {
            showNotification('All visitors approved!', 'success');
